...
- 🖼️ **静止画読み込み機能（画像オーバーレイ）**
  ...
  - 静止画表示中は画面キャプチャを自動停止するため CPU/GPU 負荷はゼロ。ファイルを排他ロックしない安全設計です。
  - いつでも「🎥 画面キャプチャに戻す」や「📐 トリミング範囲を再設定」から通常のキャプチャ表示にワンクリックで復帰できます。
- 🖥️ **デュアルディスプレイ・マルチモニター完全対応**
  - メインモニターの左・右・上・下に配置されたセカンダリディスプレイ（負の仮想スクリーン座標系）のキャプチャに完全対応。
  - 隣のモニター上の領域をトリミングしたり、オーバーレイを別モニターへ移動・保存することが可能です。
- 🎨 **多重起動モード ＆ 各枠の完全独立動作（トリミング範囲・静止画・倍率・FPS）**
  - 右クリックメニューの「多重起動」から「黄枠」「赤枠」を追加可能。
  - **各枠（青・黄・赤）は「トリミング範囲」「静止画パス」「表示倍率」「更新レート」を完全に独立して保持します。**
  - 表示倍率は **50%〜200%（50%, 60%, 70%, 80%, 90%, 100%, 125%, 150%, 175%, 200%）** に対応。100%以下の縮小表示ができるため、画面の邪魔にならずコンパクトに配置できます。
  - 「青枠にタイマーキャプチャ（125% / 60FPS）」「黄枠に目標タイム表の静止画像（80%）」「赤枠に攻略マップ画像（60%）」のように、枠ごとに最適な設定で同時表示が可能です。
  - 右クリックメニューを開くと、クリックした枠のテーマカラー（薄い青・黄・赤）が各設定項目に上品に反映され、どの枠の設定かが一目でわかります。
- 🪟 **邪魔にならない最前面・角丸四隅透過UI**
...
- 🖱️ **マウスドラッグで自由配置 ＆ 位置自動保存**
  - オーバーレイウィンドウのどこを掴んでも、お好みの場所（画面中央下や端、別モニターなど）へドラッグ移動できます。移動した位置は自動保存され、次回起動時も同じ場所に表示されます。
...
- ❌ **確実に終了できる安心設計**
  - アクティブ時に右上に表示される [×] ボタンをクリックで終了（サブ枠は個別終了）。
  - 右クリックメニューの「❌ アプリを終了」で全枠を一括終了可能。
