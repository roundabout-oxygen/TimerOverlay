...
| **右上の [×]** | アプリの終了 | 右上に常時表示されている小さな赤い [×] をクリックすると一瞬で終了します。 |
| **タスクバー** | ウィンドウを閉じる | タスクバーのアイコンを右クリック →「ウィンドウを閉じる」でも確実に終了できます。 |
| **右クリック** | 設定メニュー | 表示倍率（100%〜200%）、更新FPS（30〜120fps）、中央リセット、終了などのメニューが開きます。 |
| **F9** | 範囲の再設定 | いつでもトリミング範囲を再指定できます（オーバーレイ左上の [📐] ボタンをクリックしても可）。 |
